    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/study/time/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.quesType = data.time.quesType
                this.dataForm.memberId = data.time.memberId
                this.dataForm.totalTime = data.time.totalTime
                this.dataForm.delFlag = data.time.delFlag
                this.dataForm.createTime = data.time.createTime
                this.dataForm.updateTime = data.time.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/study/time/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
